...
// TEST COMPLET - Diagnostic du problème
router.get('/test-complete', authenticateToken, async (req, res) => {
  try {
    console.log('🔍 TEST COMPLET - Diagnostic du problème');

    // Test 1: Vérifier un employé spécifique
    console.log('\n1. 🧪 Test employé spécifique:');
    const employeTest = await pool.query(
      'SELECT id, nom, prenom, mail_responsable1, mail_responsable2 FROM employees WHERE id = 16'
    );
    
    if (employeTest.rows.length > 0) {
      const emp = employeTest.rows[0];
      console.log('   ✅ Employé trouvé:', `${emp.prenom} ${emp.nom}`);
      console.log('   📧 mail_responsable1:', emp.mail_responsable1);
      console.log('   📧 mail_responsable2:', emp.mail_responsable2);
      console.log('   🔍 Type mail_responsable1:', typeof emp.mail_responsable1);
      console.log('   🔍 Type mail_responsable2:', typeof emp.mail_responsable2);
    } else {
      console.log('   ❌ Employé non trouvé');
    }

    // Test 2: Vérifier la jointure avec une demande spécifique
    console.log('\n2. 🧪 Test jointure complète:');
    const jointureTest = await pool.query(`
      SELECT 
        d.id as demande_id,
        d.employe_id,
        e.id as employe_id,
    ...
    if (jointureTest.rows.length > 0) {
      const row = jointureTest.rows[0];
      console.log('   ✅ Jointure réussie');
      console.log('   📋 Données complètes:', row);
      console.log('   📧 mail_responsable1 dans jointure:', row.mail_responsable1);
      console.log('   📧 mail_responsable2 dans jointure:', row.mail_responsable2);
    } else {
      console.log('   ❌ Aucune donnée dans la jointure');
    }

    // Test 3: Vérifier la requête exacte utilisée dans l'API
    console.log('\n3. 🧪 Test requête API exacte:');
    const apiQueryTest = await pool.query(`
      SELECT 
        d.id,
        d.type_demande,
        d.titre,
    ...
    if (apiQueryTest.rows.length > 0) {
      const apiRow = apiQueryTest.rows[0];
      console.log('   ✅ Requête API réussie');
      console.log('   📋 Toutes les colonnes:', Object.keys(apiRow));
      console.log('   📧 mail_responsable1:', apiRow.mail_responsable1);
      console.log('   📧 mail_responsable2:', apiRow.mail_responsable2);
      
      // Vérification spécifique
      console.log('   🔍 mail_responsable1 est undefined?', apiRow.mail_responsable1 === undefined);
      console.log('   🔍 mail_responsable2 est undefined?', apiRow.mail_responsable2 === undefined);
      console.log('   🔍 mail_responsable1 est null?', apiRow.mail_responsable1 === null);
      console.log('   🔍 mail_responsable2 est null?', apiRow.mail_responsable2 === null);
    }

    res.json({
      message: 'Test complet terminé',
      employe: employeTest.rows[0],
      jointure: jointureTest.rows[0],
      api_query: apiQueryTest.rows[0]
    });
